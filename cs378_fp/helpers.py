...
# plots a graph of the loss for a trainer object
def plot_loss(trainer):
    log_history = trainer.state.log_history
    plt.plot(log_history['loss'])
    plt.title('Training Loss against Step')
    plt.xlabel('Step')
    plt.ylabel('Loss')
    plt.show()

# Does a inner join based on guid on dataset 1 and dataset 2
def inner_join_by_guid(dataset1, dataset2):
    dataset1 = dataset1.map(lambda x: {'guid': x['guid']}, remove_columns=dataset1.column_names)
    dataset2 = dataset2.map(lambda x: {'guid': x['guid']}, remove_columns=dataset2.column_names)
    dataset = dataset1.join(dataset2, on='guid', inner=True)
    return dataset
